...
export default function ClassroomStudentShell() {
  ...
  const tryAgain = (checkId: string) => {
    setTab('practice')
    setMessage('Use your teacher\'s feedback, then make a new attempt. Your earlier work stays saved.')
    requestAnimationFrame(() => document.getElementById(`practice-${checkId}`)?.scrollIntoView({block:'start'}))
  }
