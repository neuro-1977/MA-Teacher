...
export function LearningProgressPanel() {
  ...
  return <section id="workspace-progress" className="progress-shell" aria-labelledby="progress-title">
    ...
    <details className="progress-ledger"><summary>Teacher details: open the full work record</summary><div><h3>Saved work and review evidence</h3>{entries.length === 0 ? <p>No matching work.</p> : entries.map(value => <article key={value.attemptId}><header><div><strong>{value.learnerDisplayName} · {value.subject}</strong><span>{value.lessonTitle} · {value.learningStage}</span></div><em>{value.reviewState}{value.outcome ? ` · ${value.outcome}` : ''}</em></header><dl><dt>Question</dt><dd>{value.prompt}</dd><dt>What to look for</dt><dd>{value.successCriteria}</dd><dt>Answer</dt><dd>{value.responseText}</dd>{value.feedback && <><dt>Feedback</dt><dd>{value.feedback}</dd></>}</dl><footer><span>{value.evidenceCount} curriculum link{value.evidenceCount === 1 ? '' : 's'}</span><strong>{value.evidenceNeed}</strong></footer></article>)}</div></details>
  </section>;
}
